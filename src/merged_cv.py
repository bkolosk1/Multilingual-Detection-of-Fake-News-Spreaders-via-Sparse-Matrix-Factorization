...
#                svc = svm.SVC()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(config.PATH_IMGS+"/tfidif-merged-cv-expanded2.png",dpi = 300)
...
def export():
    ...
    with open(os.path.join(config.PATH_MODELS,"tokenizer_cv_en.pkl"),mode='wb') as f:
        pickle.dump(tokenizer,f)
    with open(os.path.join(config.PATH_MODELS,"clf_cv_en.pkl"),mode='wb') as f:
        pickle.dump(clf,f)
    with open(os.path.join(config.PATH_MODELS,"reducer_cv_en.pkl"),mode='wb') as f:
        pickle.dump(reducer,f)
    #ES
    XRaw,YRaw = parse_data.export(config.PATH_DATA_ES,config.PATH_DATA_ES_TRUTH)
    tokenizer, clf, reducer = train(XRaw, YRaw)
    with open(os.path.join(config.PATH_MODELS,"tokenizer_cv_es.pkl"),mode='wb') as f:
        pickle.dump(tokenizer,f)
    with open(os.path.join(config.PATH_MODELS,"clf_cv_es.pkl"),mode='wb') as f:
        pickle.dump(clf,f)
    with open(os.path.join(config.PATH_MODELS,"reducer_cv_es.pkl"),mode='wb') as f:
        pickle.dump(reducer,f)

def _import(lang,path_in=config.PATH_MODELS):
    """Imports tokenizer,clf,reducer from param(path_in, default is ../models)"""
    tokenizer = pickle.load(open(os.path.join(path_in,"tokenizer_"+lang+"_cv.pkl"),'rb'))
    clf = pickle.load(open(os.path.join(path_in,"clf_"+lang+"_cv.pkl"),'rb'))
    reducer = pickle.load(open(os.path.join(path_in,"reducer_"+lang+"_cv.pkl"),'rb'))
