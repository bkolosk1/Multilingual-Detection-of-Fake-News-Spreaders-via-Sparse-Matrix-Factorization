...
#                svc = svm.SVC()
                ...
                logging.info(str(max(clf1.cv_results_['mean_test_score'])) +" training configuration with best score (SGD, {})".format(dim))
                ...
                logging.info(str(max(clf2.cv_results_['mean_test_score'])) + " training configuration with best score (LR,{})".format(dim))
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(config.PATH_IMGS+"/tfidif-expanded.png",dpi = 300)
